...
| Persona | Core question | Planned first route |
|---|---|---|
...
These Operations routes remain planned until `E07`; they are not public proof in
the E00–E03 rollout. The planned control-evaluation journey is: architecture →
authorization hook (`prepare_action/3`) → causal Signal Journal → telemetry and
redaction → operations proof (failure drill, incident playbooks). The journey
follows the operational-control pillars in `specs/positioning.md` §11 and stays
within the claim boundaries (no IAM/compliance overreach).
